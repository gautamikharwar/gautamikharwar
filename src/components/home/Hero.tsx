    ...
    <section className="border-b border-border">
      <Container className="grid gap-12 py-20 md:grid-cols-[1.1fr_0.9fr] md:py-28">
        <div className="flex flex-col gap-6">
          <span className="inline-flex w-fit items-center gap-2 rounded-full border border-border px-4 py-1.5 text-xs text-muted">
          ...
          <div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {featuredTools.map((tool) => (
                <span key={tool} className="rounded-full border border-border bg-background px-3 py-1 text-xs">
                  {tool}
                </span>
              ))}
            </div>
